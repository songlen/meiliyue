...
class Index extends Base {
    ...
    public function index(){
        ...
        $field = 'user_id, head_pic, nickname, active_time';
        ...
        if($order_type == '2'){
            ...
            // sql 计算距离 并按距离排序
            $field .= ", longitude, latitude, ROUND(6378.138*2*ASIN(SQRT(POW(SIN(($user_latitude*PI()/180-latitude*PI()/180)/2),2)+COS($user_latitude*PI()/180)*COS(latitude*PI()/180)*POW(SIN(($user_longitude*PI()/180-longitude*PI()/180)/2),2))), 2) AS distance";
            $order = 'distance asc';
        }
        // 条件
        $where = array(
